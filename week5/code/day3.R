...
# finding the covariance between 2 variables
rm(list = ls())
par(mfrow = c(1,3))
x<- c(-10:10)
var(x)
...
cov(x,y2)
plot(x,y2,xlim=c(-10,10),ylim=c(-20,20), col="red", pch=19, cex=0.8, main=paste("Cov=", round(cov(x,y2), digits = 2)))

y3<-x*(-1)+rnorm(21,mean = 0, sd=sqrt(1))
cov(x,y3)
plot(x,y3,xlim=c(-10,10),ylim=c(-20,20), col="red", pch=19, cex=0.8, main=paste("Cov=", round(cov(x,y3), digits = 2)))
